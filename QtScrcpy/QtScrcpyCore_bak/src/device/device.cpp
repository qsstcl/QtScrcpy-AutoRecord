...
void Device::screenshotWithFilename(const QString &filename)
{
    if (!m_decoder) {
        return;
    }
    ...
    // screenshot
    m_decoder->peekFrame([&](int width, int height, uint8_t* dataRGB32) {
        saveFrameWithFilename(width, height, dataRGB32, filename);
    });
}
...
bool Device::saveFrameWithFilename(int width, int height, uint8_t *dataRGB32, const QString &filename)
{
    if (!dataRGB32) {
        return false;
    }
    ...
    if (!dir.exists(fileInfo.dir().path()))
        dir.mkpath(fileInfo.dir().path());

    qInfo() << "try save to " << absFilePath;
    int ret = rgbImage.save(absFilePath, "JPEG", 100);
    if (!ret) {
        return false;
    }
